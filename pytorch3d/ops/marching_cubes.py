...
from pytorch3d.ops.marching_cubes_data import (
    EDGE_TABLE,
    EDGE_TO_VERTICES,
    FACE_TABLE,
)
...
def marching_cubes_naive(
    ...
    for i in range(len(volume_data_batch)):
        ...
        if len(faces) > 0 and len(verts) > 0:
            ...
            if return_local_coords:
                verts = world_to_local_transform.transform_points(
                    verts[None, ...]
                )
                verts = verts.squeeze()
...
def _calculate_interp_vertices(
    ...
    for edge_index in edge_indices:
        ...
        if (p_tuple1, p_tuple2) in edge_vertices_to_index:
            ...
            val1, val2 = (
                _get_value(point1, volume_data),
                _get_value(point2, volume_data),
            )
