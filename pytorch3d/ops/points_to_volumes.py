...
def add_pointclouds_to_volumes(
    ...
    mask = torch.arange(
        n_per_pcl.max(), dtype=pcl_feats.dtype, device=pcl_feats.device
    )
...
def add_points_features_to_volume_densities_features(
    ...
    if volume_features is None:
        ...
        volume_features_flatten = volume_densities.new_zeros(
            ba, feature_dim, n_voxels
        )
    else:
        # otherwise just flatten
        volume_features_flatten = volume_features.view(
            ba, feature_dim, n_voxels
        )
    ...
    for xdiff in (0, 1):
        ...
        for ydiff in (0, 1):
            ...
            for zdiff in (0, 1):
                ...
                idx_valid = idx_valid.view(ba, 1, n_points).expand_as(
                    points_features
                )
                ...
                volume_features.scatter_add_(
                    2, idx_valid, w_valid * points_features
                )

    # divide each feature by the total weight of the votes
    # pyre-fixme[20]: Argument `max` expected.
    volume_features = volume_features / volume_densities.view(
        ba, 1, n_voxels
    ).clamp(min_weight)
