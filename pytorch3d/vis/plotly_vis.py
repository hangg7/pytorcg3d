    ...
    for subplot_idx in range(len(subplots)):
        ...
        for trace_name, struct in traces.items():
            if isinstance(struct, Meshes):
                _add_mesh_trace(
                    fig, struct, trace_name, subplot_idx, ncols, lighting
                )
...
def plot_batch_individually(
    batched_structs: Union[
        List[Union[Meshes, Pointclouds, CamerasBase]],
        Meshes,
        Pointclouds,
        CamerasBase,
    ],
    ...
    if isinstance(batched_structs, list):
        ...
        for struct in batched_structs:
            if len(struct) not in (1, max_size):
                msg = 'invalid batch size {} provided: {}'.format(
                    len(struct), struct
                )
...
def _add_struct_from_batch(
    ...
    scene_dictionary: Dict[
        str, Dict[str, Union[CamerasBase, Meshes, Pointclouds]]
    ],
    trace_idx: int = 1,
):
    """
...
def _add_pointcloud_trace(
    ...
    if verts.shape[0] > total_points_count:
        indices = np.random.choice(
            verts.shape[0], total_points_count, replace=False
        )
        verts = verts[indices]
