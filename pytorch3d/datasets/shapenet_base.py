...
class ShapeNetBase(torch.utils.data.Dataset):
        ...
        **kwargs
        ...
        if meshes.textures is None:
            meshes.textures = TexturesVertex(
                verts_features=torch.ones_like(
    ...
    def _handle_render_inputs(
        ...
        if model_ids is not None and len(model_ids) > 0:
            ...
            for model_id in model_ids:
                if model_id not in self.model_ids:
                    raise ValueError(
                        'model_id %s not found in the loaded dataset.'
                        % model_id
                    )
        ...
        # the corresponding indices.
            ...
            for i in range(len(categories)):
                ...
                sample_num = (
                    sample_nums[i] if len(sample_nums) > 1 else sample_nums[0]
                )
    ...
    def _sample_idxs_from_category(
        ...
        range_len = (
            self.synset_num_models[category]
