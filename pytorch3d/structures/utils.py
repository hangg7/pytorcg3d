...
def list_to_padded(
    ...
    x = [
        ...
        (
            y.new_zeros([0] * element_ndim)
            if (y.ndim == 1 and y.nelement() == 0)
            else y
        )
        for y in x
    ]
    ...
    if any(y.ndim != x[0].ndim for y in x):
        raise ValueError(
            'All items have to have the same number of dimensions!'
        )

    if pad_size is None:
        pad_dims = [
            max(y.shape[dim] for y in x if len(y) > 0)
            for dim in range(x[0].ndim)
        ]
    else:
        if any(len(pad_size) != y.ndim for y in x):
            raise ValueError(
                'Pad size must contain target size for all dimensions.'
            )
        pad_dims = pad_size
