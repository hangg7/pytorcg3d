...
class Volumes(object):
    ...
    def _handle_voxel_size(self, voxel_size, batch_size):
        ...
        return self._convert_volume_property_to_tensor(
            voxel_size, batch_size, err_msg
        )
    ...
    def _handle_volume_translation(self, translation, batch_size):
        ...
        return self._convert_volume_property_to_tensor(
            translation, batch_size, err_msg
        )
    ...
    def get_coord_grid(self, world_coordinates: bool = True) -> torch.Tensor:
        ...
        return self._calculate_coordinate_grid(
    ...
    def _calculate_coordinate_grid(
        ...
        vol_axes = [
            torch.linspace(
                -1.0, 1.0, r, dtype=torch.float32, device=self.device
            )
            for r in (de, he, wi)
        ]
        ...
        vol_coords_local = torch.stack((X, Y, Z), dim=3)[None].repeat(
            ba, 1, 1, 1, 1
        )
    ...
    def world_to_local_coords(
        self, points_3d_world: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def local_to_world_coords(
        self, points_3d_local: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def _set_densities_features(self, x, var_name):
        x_tensor, grid_sizes = self._convert_densities_features_to_tensor(
            x, var_name
        )
