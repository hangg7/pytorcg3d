...
from pytorch3d.structures.utils import (
    list_to_packed,
    list_to_padded,
    padded_to_list,
)
...
def _list_to_padded_wrapper(
    ...
    for y in x:
        if y.ndim != dims and y.shape[1:] != reshape_dims:
            msg = 'list_to_padded requires tensors to have the same number of dimensions'
            raise ValueError(msg)
        x_reshaped.append(y.reshape(-1, D))
    x_padded = list_to_padded(
        x_reshaped, pad_size=pad_size, pad_value=pad_value
    )
...
class TexturesAtlas(TexturesBase):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(atlas, list):
            ...
            raise ValueError(
                'Not all values are provided in the correct format'
            )
    ...
    def atlas_list(self) -> List[torch.Tensor]:
        if self._atlas_list is None:
            if self.isempty():
                self._atlas_padded = [
                    torch.empty(
                        (0, 0, 0, 3), dtype=torch.float32, device=self.device
                    )
                ] * self._N
    ...
    def sample_textures(self, fragments, **kwargs) -> torch.Tensor:
        ...
        w_xy = (
            (bary_w01 * R).to(torch.int64).clamp(max=R - 1)
        )  # (N, H, W, K, 2)
...
class TexturesUV(TexturesBase):
    def __init__(
        ...
        if isinstance(verts_uvs, (list, tuple)):
            ...
            if not all(v.device == self.device for v in verts_uvs):
                raise ValueError(
                    'verts_uvs and faces_uvs must be on the same device'
                )
        ...
        if torch.is_tensor(maps):
            ...
            if len(maps) != self._N:
                raise ValueError(
                    'Expected one texture map per mesh in the batch.'
                )
        ...
        if self._maps_padded.device != self.device:
            raise ValueError(
                'maps must be on the same device as verts/faces uvs.'
            )
    ...
    def __getitem__(self, index):
        ...
        if all(
            isinstance(f, (list, tuple)) for f in [faces_uvs, verts_uvs, maps]
        ):
    ...
    def join_batch(self, textures: List['TexturesUV']) -> 'TexturesUV':
        ...
        if not align_corners_same:
            raise ValueError(
                'All textures must have the same align_corners value.'
            )
...
class TexturesVertex(TexturesBase):
    def __init__(
        ...
        if isinstance(verts_features, (tuple, list)):
            ...
            raise ValueError(
                'verts_features must be a tensor or list of tensors'
            )
    ...
    def clone(self):
        ...
        if self._verts_features_list is not None:
            tex._verts_features_list = [
                f.clone() for f in self._verts_features_list
            ]
        num_verts = (
    ...
    def detach(self):
        ...
        if self._verts_features_list is not None:
            tex._verts_features_list = [
                f.detach() for f in self._verts_features_list
            ]
        num_verts = (
    ...
    def verts_features_packed(self) -> torch.Tensor:
        if self.isempty():
            return torch.zeros(
                (self._N, 3, 0), dtype=torch.float32, device=self.device
            )
    ...
    def extend(self, N: int) -> 'TexturesVertex':
        new_props = self._extend(
            N, ['verts_features_padded', '_num_verts_per_mesh']
        )
        new_tex = TexturesVertex(
            verts_features=new_props['verts_features_padded']
        )
        new_tex._num_verts_per_mesh = new_props['_num_verts_per_mesh']
        return new_tex
    ...
    def join_batch(self, textures: List['TexturesVertex']) -> 'TexturesVertex':
        ...
        """
        tex_types_same = all(
            isinstance(tex, TexturesVertex) for tex in textures
        )
        if not tex_types_same:
    ...
    def join_scene(self) -> 'TexturesVertex':
        ...
        """
        return self.__class__(
            verts_features=[torch.cat(self.verts_features_list())]
        )
