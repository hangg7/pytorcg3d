...
class ImplicitRenderer(torch.nn.Module):
    ...
    def forward(
        ...
        if not callable(volumetric_function):
            raise ValueError(
                '"volumetric_function" has to be a "Callable" object.'
            )
...
class VolumeSampler(torch.nn.Module):
    ...
    def __init__(self, volumes: Volumes, sample_mode: str = 'bilinear'):
        ...
        if not isinstance(volumes, Volumes):
            raise ValueError(
                "'volumes' have to be an instance of the 'Volumes' class."
            )
    ...
    def _get_ray_directions_transform(self):
        ...
        """
        world2local = (
            self._volumes.get_world_to_local_coords_transform().get_matrix()
        )
    ...
    def forward(
        ...
        if self._volumes.densities().shape[0] != rays_origins_world.shape[0]:
            raise ValueError(
                'Input volumes have to have the same batch size as rays.'
            )
        ...
        rays_origins_local = self._volumes.world_to_local_coords(
            rays_origins_world
        )
        ...
        if volumes_features is None:
            ...
            data_to_sample = torch.cat(
                (volumes_densities, volumes_features), dim=1
            )
