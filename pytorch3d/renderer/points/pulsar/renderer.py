...
class Renderer(torch.nn.Module):
    ...
    @staticmethod
    def sphere_ids_from_result_info_nograd(
        result_info: torch.Tensor
    ) -> torch.Tensor:
        """
        Get the sphere IDs from a result info tensor.
        """
        if result_info.ndim == 3:
            return Renderer.sphere_ids_from_result_info_nograd(
                result_info[None, ...]
            )
        # pyre-fixme[16]: Module `pytorch3d` has no attribute `_C`.
    ...
    @staticmethod
    def depth_map_from_result_info_nograd(
        result_info: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def forward(
        ...
        if (
            ...
            and max_depth > 10000.0 * focal_lengths.min().item()
        ):
            warnings.warn(
                (
