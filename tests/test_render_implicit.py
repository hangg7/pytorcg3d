...
def spherical_volumetric_function(
    ...
    **kwargs,
    ...
    rays_densities = torch.sigmoid(
        -100.0 * (surface_dist - sphere_diameter ** 2)
    )
...
class TestRenderImplicit(TestCaseMixin, unittest.TestCase):
    ...
    def renderer(
        ...
        sphere_centroid = (
            torch.randn(batch_size, 3, device=cameras.device) * 0.1
        )
    ...
    def test_compare_with_meshes_renderer(
        ...
        verts = (
            torch.nn.functional.normalize(ico.verts_padded(), dim=-1)
            * sphere_diameter
        ...
        if DEBUG:
            ...
            for (image_opacity, image_opacity_mesh) in zip(
                ...
                image_mesh, opacity_mesh = image_opacity_mesh.split(
                    [3, 1], dim=-1
                )
                diff_image = (
            ...
            image_pil = Image.fromarray(
                (
                    torch.cat(
                        (
                            image,
                            opacity[..., None].clamp(0.0, 1.0).repeat(1, 1, 3),
                        ),
                        dim=1,
                    )
                    .detach()
