...
class TestRaysampling(TestCaseMixin, unittest.TestCase):
    ...
    def raysampler(
        ...
        cameras = init_random_cameras(
            camera_type, batch_size, random_z=True
        ).to(device)
    ...
    def _check_grid_shape(
        self, grid, batch_size, spatial_size, n_pts_per_ray, dim
    ):
        """
        ...
        self.assertTrue(
            all(sz1 == sz2 for sz1, sz2 in zip(grid.shape, tgt_shape))
        )
    ...
    def _check_raysampler_output_shapes(
        ...
        self._check_grid_shape(
            ray_bundle.origins, batch_size, spatial_size, 0, 3
        )
        self._check_grid_shape(
            ray_bundle.directions, batch_size, spatial_size, 0, 3
        )
        self._check_grid_shape(
    ...
    def _check_raysampler_ray_points(
        ...
        self.assertClose(
            ray_bundle.xys.view(batch_size, -1, 1, 2).expand_as(
                rays_xy_projected
            ),
            rays_xy_projected,
            atol=1e-4,
        ...
        if isinstance(raysampler, GridRaysampler):
            ...
            for dim, axis_bounds in zip(
                (0, 1), ((min_x, max_x), (min_y, max_y))
            ):
                self.assertTrue(
                    (
    ...
    def _check_raysampler_ray_directions(self, cameras, raysampler, ray_bundle):
        ...
        rays_points_world = cameras.unproject_points(
            torch.cat(
                (
                    ray_bundle.xys.view(
                        batch_size, n_rays_per_image, 1, 2
                    ).expand(batch_size, n_rays_per_image, n_pts_per_ray, 2),
        ...
        self.assertClose(
            rays_directions_world_normed[:, :, None].expand_as(
        ...
        with torch.random.fork_rng(devices=range(torch.cuda.device_count())):
            ...
            torch.random.manual_seed(42)
            ray_bundle_camera_fix_seed = raysampler(
                cameras=cameras_trivial_extrinsic
            )

        rays_directions_camera_fix_seed_ = Rotate(
            cameras.R, device=cameras.R.device
        ).transform_points(
            ray_bundle_world_fix_seed.directions.view(batch_size, -1, 3)
        )
