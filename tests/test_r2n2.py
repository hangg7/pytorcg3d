...
class TestR2N2(TestCaseMixin, unittest.TestCase):
    ...
    def test_load_R2N2(self):
        ...
        for synset in split_dict:
            ...
            synset_views_list = r2n2_dataset.views_per_model_list[
                idx_start:idx_end
            ]
        ...
        self.assertEqual(
            r2n2_dataset[39, torch.tensor([12, 21])]['images'].shape[0], 2
        )
    ...
    def test_collate_models(self):
        ...
        self.assertEqual(
            object_batch['mesh'].verts_padded().shape[0], batch_size
        )
        self.assertEqual(
    ...
    def test_render_r2n2(self):
        ...
        for idx in range(3):
            ...
            if DEBUG:
                Image.fromarray(
                    (r2n2_by_idxs_rgb.numpy() * 255).astype(np.uint8)
                ).save(DATA_DIR / ('DEBUG_r2n2_render_by_idxs_%s.png' % idx))
        ...
        # Compare the rendered models to the reference images.
        for idx in range(3):
            r2n2_by_model_ids_rgb = (
                r2n2_by_model_ids[idx, ..., :3].squeeze().cpu()
            )
            if DEBUG:
                Image.fromarray(
                    (r2n2_by_model_ids_rgb.numpy() * 255).astype(np.uint8)
                ).save(
                    DATA_DIR / ('DEBUG_r2n2_render_by_model_ids_%s.png' % idx)
                )
            image_ref = load_rgb_image(
                'test_r2n2_render_by_idxs_and_ids_%s.png' % idx, DATA_DIR
            )
    ...
    def test_render_by_r2n2_calibration(self):
        ...
        for idx in range(4):
            ...
            if DEBUG:
                Image.fromarray(
                    (r2n2_batch_rgb.numpy() * 255).astype(np.uint8)
                ).save(
                    DATA_DIR
                    / (
    ...
    def test_render_voxels(self):
        ...
        if DEBUG:
            Image.fromarray(
                (vox_render_rgb.numpy() * 255).astype(np.uint8)
            ).save(DATA_DIR / ('DEBUG_r2n2_voxel_to_mesh_render.png'))
        image_ref = load_rgb_image(
            'test_r2n2_voxel_to_mesh_render.png', DATA_DIR
        )
