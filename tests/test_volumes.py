...
class TestVolumes(TestCaseMixin, unittest.TestCase):
    ...
    def _check_indexed_volumes(self, v, selected, indices):
        for selectedIdx, index in indices:
            self.assertClose(
                selected.densities()[selectedIdx], v.densities()[index]
            )
            self.assertClose(
            ...
            if selected.features() is not None:
                self.assertClose(
                    selected.features()[selectedIdx], v.features()[index]
                )
    ...
    def test_get_item(
        ...
        densities = torch.randn(
            size=[num_volumes, 1, *volume_size],
            device=device,
            dtype=torch.float32,
        )
        ...
        for features_, densities_ in zip(
            ...
            self._check_indexed_volumes(
                v, v_selected, enumerate(index.tolist())
            )
    ...
    def test_coord_transforms(
        ...
        for _ in range(10):
            ...
            local_offset = torch.tensor(
                list(size), dtype=torch.float32, device=device
            )[[2, 1, 0]][None].repeat(num_volumes, 1)
            ...
            grid_rot_trans_vs = v_trans_vs.get_coord_grid(
            ...
            grid_default_local = v_default.get_coord_grid(
    ...
    def test_coord_grid_convention(
        ...
        pts_world = torch.tensor(
            [-2, 3, -2], device=device, dtype=torch.float32
    ...
    def test_coord_grid_convention_heterogeneous(
        ...
        densities_list = [
            torch.randn(
                size=[num_channels, *size], device=device, dtype=torch.float32
            )
            for size in sizes
        ]
        ...
        # resample grid_world using grid_sampler with local coords
        # -> make sure the resampled version is the same as original
        for grid_world_, grid_local_, size in zip(
            grid_world, grid_local, sizes
        ):
            ...
            self.assertClose(
                grid_world_crop_resampled, grid_world_crop, atol=1e-7
            )
    ...
    def test_coord_grid_transforms(
        ...
        for _ in range(10):
            ...
            center = torch.randn(
                num_volumes, 3, dtype=torch.float32, device=device
            )
            voxel_size = (
                torch.rand(1, dtype=torch.float32, device=device) * 5.0 + 0.5
            )
    ...
    def test_clone(
        ...
        for has_features in (True, False):
            ...
            self.assertNotAlmostEqual(
                float(
                    (
                        vnew.densities()[0, 0, 0, 0, 0]
                        - v.densities()[0, 0, 0, 0, 0]
                    )
                    .abs()
                    .max()
                ),
            ...
            if has_features:
                ...
                self.assertNotAlmostEqual(
                    float(
                        (
                            vnew.features()[0, 0, 0, 0, 0]
                            - v.features()[0, 0, 0, 0, 0]
                        )
                        .abs()
                        .max()
                    ),
    ...
    def test_to(
        ...
        densities = torch.rand(
            size=[num_volumes, 1, *size], device=device, dtype=dtype
        )
    ...
    def test_feature_density_setters(self):
        ...
        for _ in range(num_volumes):
            ...
            diff_grid_size = (
                copy.deepcopy(grid_size)
                + torch.randint(2, size=(3,)).long()
                + 1
            )
        ...
        bad_densities_new = [
            [
                torch.rand(
                    (1, *grid_size), device=diff_device, dtype=torch.float32
                )
                for grid_size in diff_grid_sizes
            ],
            torch.rand(
                (num_volumes, 1, K + 1, K, K),
                device=device,
                dtype=torch.float32,
            ),
            None,
        ...
        for x_pad, x_list in zip(
            ...
            self._check_padded(x_pad, x_list, grid_sizes)
        self.assertIs(
            volumes.get_grid_sizes(), volumes_updated.get_grid_sizes()
        )
        self.assertIs(
    ...
    def test_constructor(
        ...
        def zip_with_ok_indicator(good, bad):
            return zip(
                [*good, *bad], [*([True] * len(good)), *([False] * len(bad))]
            )
