        ...
        for device in [torch.device('cpu'), torch.device('cuda')]:
            ...
            for azimuth in [0.0, 90.0]:
                ...
                for camera_name, cameras in [
                    (
                        'fovperspective',
                        FoVPerspectiveCameras(device=device, R=R, T=T),
                    ),
                    (
                        'fovorthographic',
                    ...
                    (
                        'perspective',
                        PerspectiveCameras(device=device, R=R, T=T),
                    ),
                    (
                        'orthographic',
                        OrthographicCameras(device=device, R=R, T=T),
                    ),
                ]:
                    raster_settings = PointsRasterizationSettings(
                        image_size=256, radius=5e-2, points_per_pixel=1
                    ...
                    renderer = PulsarPointsRenderer(rasterizer=rasterizer).to(
                        device
                    )
                    # Load reference image
                    filename = (
        ...
        for device in [torch.device('cpu'), torch.device('cuda')]:
            ...
            for _, cameras in [
                ...
                for image_size in (256, (256, 256)):
                    ...
                    if isinstance(
                        cameras, (PerspectiveCameras, OrthographicCameras)
                    ):
