...
def create_multiview():
    ...
    class Model(nn.Module):
        ...
        def forward(self, vp, vc, vr, cam_params):
            ...
            return self.renderer.forward(
                vp, vc, vr, cam_params, self.gamma, 45.0
            )
    ...
    for device in [torch.device('cuda')]:
        ...
        for angle_idx, angle in enumerate(
            [-1.5, -0.8, -0.4, -0.1, 0.1, 0.4, 0.8, 1.5]
        ):
            ...
            result = model.forward(
                vert_pos_v, vert_col_v, vert_rad_v, cam_params_v
            )
            result_im = (result.cpu().detach().numpy() * 255).astype(np.uint8)
            imageio.imsave(
                'reference/examples_TestRenderer_test_multiview_%d.png'
                % (angle_idx),
                result_im,
            )
