...
class SceneModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_parameter(
            'vert_col',
            nn.Parameter(
                torch.zeros(1, n_points, 3, dtype=torch.float32),
                requires_grad=True,
            ),
        )
        self.register_parameter(
        ...
        self.register_parameter(
            'vert_opy',
            nn.Parameter(
                torch.ones(1, n_points, dtype=torch.float32),
                requires_grad=False,
            ),
        )
        ...
        for device in devices:
            ...
            for _ in range(50):
                ...
                for idx in range(n_points):
                    self.assertTrue(
                        (sphere_ids == idx).sum() > 0,
                        'Sphere ID %d missing!' % (idx),
                    )
                ...
                self.assertTrue(
                    torch.all(model.vert_col.grad[:, :, 0].abs() > 0.0)
                )
                angle += 0.15
