...
class TestRaymarching(TestCaseMixin, unittest.TestCase):
    ...
    def _init_random_rays(
        ...
        rays_z = torch.arange(n_pts_per_ray, dtype=dtype, device=device)[
            None
        ].repeat(n_rays, 1)
        ...
        rays_densities = (rays_z >= depths_gt[..., None]).type_as(rays_z)[
            ..., None
        ]
        ...
        gt_surface = ((rays_z - depths_gt[..., None]).abs() <= 1e-4).type_as(
            rays_z
        )
    ...
    def test_emission_absorption(self):
        ...
        features_render, opacities_render = (
            data_render[..., :-1],
            data_render[..., -1],
        )
        ...
        self.assertClose(
            depths_render * opacities_render, depths_gt * opacities_gt
        )
