...
def _generate_data(N, S, K, F, D, device, requires_grad=False):
    ...
    face_attrs = torch.randn(
        F, 3, D, device=device, requires_grad=requires_grad
    )
...
def bm_interpolate_face_attribues() -> None:
    ...
    for case in test_cases:
        N, S, K, F, D, impl = case
        kwargs_list.append(
            {'N': N, 'S': S, 'K': K, 'F': F, 'D': D, 'impl': impl}
        )
    benchmark(_bm_forward, 'FORWARD', kwargs_list, warmup_iters=3)
    benchmark(
        _bm_forward_backward, 'FORWARD+BACKWARD', kwargs_list, warmup_iters=3
    )
