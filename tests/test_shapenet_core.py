...
class TestShapenetCore(TestCaseMixin, unittest.TestCase):
    ...
    def test_render_shapenet_core(self):
        ...
        for idx in range(3):
            ...
            if DEBUG:
                Image.fromarray(
                    (piano_rgb.numpy() * 255).astype(np.uint8)
                ).save(
                    DATA_DIR
                    / ('DEBUG_shapenet_core_render_piano_by_idxs_%s.png' % idx)
                )
        ...
        for idx in range(3):
            ...
            if DEBUG:
                Image.fromarray(
                    (piano_rgb_2.numpy() * 255).astype(np.uint8)
                ).save(
                    DATA_DIR
                    / ('DEBUG_shapenet_core_render_piano_by_ids_%s.png' % idx)
                )
            image_ref = load_rgb_image(
                'test_shapenet_core_render_piano_%s.png' % idx, DATA_DIR
            )
