...
class TestRasterizePoints(TestCaseMixin, unittest.TestCase):
    ...
    def _test_coarse_rasterize(self, device):
        ...
        pointclouds = Pointclouds(points=[points])
        radius = torch.full(
            (points.shape[0],), fill_value=radius, device=device
        )
        args = (
    ...
    def _test_variable_size_radius(
        self, rasterize_points_fn, device, bin_size=0
    ):
        # Two points
        points = torch.tensor(
        ...
        points_per_pixel = 1
        radius = torch.tensor(
            [0.1, 0.0, 0.2], dtype=torch.float32, device=device
        )
        pointclouds = Pointclouds(points=[points])
        if bin_size == -1:
            # simple python case with no binning
