...
class TestRasterizeRectangleImagesErrors(TestCaseMixin, unittest.TestCase):
    def test_mesh_image_size_arg(self):
        ...
        with self.assertRaises(ValueError) as cm:
            rasterize_meshes(meshes, (100, 200, 3), 0.0001, faces_per_pixel=1)
            self.assertTrue('tuple/list of (H, W)' in cm.msg)

        with self.assertRaises(ValueError) as cm:
            rasterize_meshes(meshes, (0, 10), 0.0001, faces_per_pixel=1)
            self.assertTrue('sizes must be positive' in cm.msg)

        with self.assertRaises(ValueError) as cm:
            rasterize_meshes(meshes, (100.5, 120.5), 0.0001, faces_per_pixel=1)
    ...
    def test_points_image_size_arg(self):
        ...
        with self.assertRaises(ValueError) as cm:
            rasterize_points(points, (100, 200, 3), 0.0001, points_per_pixel=1)
            self.assertTrue('tuple/list of (H, W)' in cm.msg)

        with self.assertRaises(ValueError) as cm:
            rasterize_points(points, (0, 10), 0.0001, points_per_pixel=1)
            self.assertTrue('sizes must be positive' in cm.msg)

        with self.assertRaises(ValueError) as cm:
            rasterize_points(points, (100.5, 120.5), 0.0001, points_per_pixel=1)
            self.assertTrue('sizes must be integers' in cm.msg)
...
class TestRasterizeRectangleImagesMeshes(TestCaseMixin, unittest.TestCase):
    ...
    def _rasterize(self, meshes, image_size, bin_size, blur):
        ...
        face_idxs, zbuf, bary_coords, pix_dists = rasterize_meshes(
            meshes, image_size, blur, faces_per_pixel=1, bin_size=bin_size
        )
        return Fragments(
    ...
    def _compare_square_with_nonsq(
        ...
        H, W = image_size
        S = min(H, W)
        verts_square, meshes_sq = self._clone_mesh(
        ...
        for fragments, grad_tensor, _name in nonsq_fragment_gradtensor_list:
            ...
            self.assertEqual(
                non_square_mask.sum().item(), square_mask.sum().item()
            )
            ...
            self.assertClose(square_bary, non_square_bary, atol=2e-7)
            ...
            self.assertClose(
                non_square_grad_dists, square_fragments.dists.grad[square_mask]
            )
            self.assertClose(
            ...
            self.assertClose(
                non_square_grad_zbuf, square_fragments.zbuf.grad[square_mask]
            )
    ...
    def test_gpu(self):
        ...
        for image_size, blur, device, batch_size in test_cases:
            ...
            fragments_naive = self._rasterize(
                meshes_nonsq_naive, image_size, blur=blur, bin_size=0
            )
            fragments_binned = self._rasterize(
                meshes_nonsq_binned, image_size, blur=blur, bin_size=None
            )

            # Save out debug images if needed
            _save_debug_image(fragments_naive.pix_to_face, image_size, 0, blur)
            _save_debug_image(
                fragments_binned.pix_to_face, image_size, None, blur
            )
    ...
    def test_cpu(self):
        ...
        for image_size, blur, device, batch_size in test_cases:
            ...
            face_idxs, zbuf, bary_coords, pix_dists = rasterize_meshes_python(
                meshes_nonsq_python, image_size, blur, faces_per_pixel=1
            )
            ...
            _save_debug_image(fragments_naive.pix_to_face, image_size, 0, blur)
            _save_debug_image(
                fragments_python.pix_to_face, image_size, 'python', blur
            )
...
class TestRasterizeRectangleImagesPointclouds(TestCaseMixin, unittest.TestCase):
    ...
    def _rasterize(self, meshes, image_size, bin_size, blur):
        ...
        idxs, zbuf, dists = rasterize_points(
            meshes, image_size, blur, points_per_pixel=1, bin_size=bin_size
        )
        return PointFragments(idx=idxs, zbuf=zbuf, dists=dists)
    ...
    def _compare_square_with_nonsq(
        ...
        grad_zbuf = torch.randn_like(square_zbuf)
        grad_dist = torch.randn_like(square_dists)
        loss0 = (grad_dist * square_dists).sum() + (
            grad_zbuf * square_zbuf
        ).sum()
        loss0.backward()
    ...
    def test_gpu(self):
        ...
        for image_size, blur, device, batch_size in test_cases:
            ...
            # Get the outputs for both naive and coarse to fine rasterization
            fragments_naive = self._rasterize(
                pointcloud_nonsq_naive, image_size, blur=blur, bin_size=0
            )
            fragments_binned = self._rasterize(
                pointcloud_nonsq_binned, image_size, blur=blur, bin_size=None
            )
    ...
    def test_cpu(self):
        ...
        for image_size, blur, device, batch_size in test_cases:
            ...
            idxs, zbuf, pix_dists = rasterize_points_python(
                pointcloud_nonsq_python, image_size, blur, points_per_pixel=1
            )
            fragments_python = PointFragments(
                idx=idxs, zbuf=zbuf, dists=pix_dists
            )
    ...
    def test_render_pointcloud(self):
        ...
        image_ref = load_rgb_image(
            'test_pointcloud_rectangle_image.png', DATA_DIR
        )
