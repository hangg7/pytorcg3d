...
from pytorch3d.renderer.mesh.renderer import (
    MeshRenderer,
...
class TestRenderMeshes(TestCaseMixin, unittest.TestCase):
        ...
        for cam_type in (
            ...
            for (name, shader_init) in shaders.items():
                ...
                if check_depth:
                    ...
                    self.assertClose(
                        fragments.zbuf, rasterizer(sphere_mesh).zbuf
                    )
                else:
    ...
    def test_simple_sphere_screen(self):
        ...
        for cam_type in (PerspectiveCameras, OrthographicCameras):
            ...
            rgb = images[0, ..., :3].squeeze().cpu()
            filename = (
                'test_simple_sphere_light_phong_%s.png' % cam_type.__name__
            )
    ...
    def test_silhouette_with_grad(self):
        ...
        for cam_type in (
            ...
            if DEBUG:
                ...
                Image.fromarray(
                    (alpha.detach().numpy() * 255).astype(np.uint8)
                ).save(filename)
    ...
    def test_batch_uvs(self):
        ...
        colors1 = torch.stack(
            [nocolor, color_gradient1, color_gradient2], dim=2
        )
        colors2 = torch.stack(
            [color_gradient1, color_gradient2, nocolor], dim=2
        )
        ...
        if DEBUG:
            ...
            Image.fromarray(
                ((diff > 1e-5).cpu().numpy().astype(np.uint8) * 255)
            ).save(DATA_DIR / 'test_batch_uvs01.png')
            diff = torch.abs(outputs[0][1, ..., :3] - outputs[2][0, ..., :3])
            Image.fromarray(
                ((diff > 1e-5).cpu().numpy().astype(np.uint8) * 255)
            ).save(DATA_DIR / 'test_batch_uvs23.png')

        self.assertClose(
            outputs[0][0, ..., :3], outputs[1][0, ..., :3], atol=1e-5
        )
        self.assertClose(
            outputs[0][1, ..., :3], outputs[2][0, ..., :3], atol=1e-5
        )
    ...
    def test_join_uvs(self):
        ...
        for i, align_corners, padding_mode in [
            ...
            mesh1 = Meshes(verts=[verts], faces=[faces], textures=textures1)
            mesh2 = Meshes(
                verts=[verts_shifted1], faces=[faces], textures=textures2
            )
            mesh3 = Meshes(
                verts=[verts_shifted2], faces=[faces], textures=textures3
            )
            ...
            if DEBUG:
                ...
                Image.fromarray(
                    (
                        mesh2.textures.maps_padded()[0].cpu().numpy() * 255
                    ).astype(np.uint8)
                ).save(DATA_DIR / f'test_joinuvs{i}_map2.png')
                Image.fromarray(
                    (
                        mesh3.textures.maps_padded()[0].cpu().numpy() * 255
                    ).astype(np.uint8)
            ...
            self.assertClose(
                mesh.textures.maps_padded()[0].cpu(), map_ref, atol=0.05
            )
    ...
    def test_join_atlas(self):
        ...
        atlas1 = torch.rand(
            size=(faces.shape[0], map_size, map_size, 3), device=device
        )
    ...
    def test_texture_map_atlas(self):
        ...
        # Init shader settings
        materials = Materials(
            device=device, specular_color=((0, 0, 0),), shininess=0.0
        )
        lights = PointLights(device=device)
        ...
        if DEBUG:
            ...
            ).save(DATA_DIR / 'DEBUG_texture_atlas_8x8_back.png')
