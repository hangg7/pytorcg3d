...
class TestTexturesVertex(TestCaseMixin, unittest.TestCase):
    ...
    def test_padded_to_packed(self):
        ...
        verts_features_list = packed_to_list(
            verts_features_packed, num_verts_per_mesh
        )
...
class TestTexturesAtlas(TestCaseMixin, unittest.TestCase):
    ...
    def test_detach(self):
        tex = TexturesAtlas(
            atlas=torch.rand(size=(1, 10, 2, 2, 3), requires_grad=True)
        )
        tex.atlas_list()
        tex_detached = tex.detach()
    ...
    def test_extend(self):
        ...
        self.assertAllSeparate(
            [tex_init.atlas_padded(), new_tex.atlas_padded()]
        )
...
class TestTexturesUV(TestCaseMixin, unittest.TestCase):
    ...
    def test_faces_verts_textures(self):
        ...
        for n in range(N):
            ...
            for f in range(F):
                ...
                idx = torch.stack((uv0, uv1, uv2), dim=0).view(
                    1, 1, 3, 2
                )  # 1x1x3x2
                idx = idx * 2.0 - 1.0
    ...
    def test_clone(self):
        ...
        for i in range(tex._N):
            self.assertSeparate(
                tex._faces_uvs_list[i], tex_cloned._faces_uvs_list[i]
            )
            self.assertClose(
                tex._faces_uvs_list[i], tex_cloned._faces_uvs_list[i]
            )
            self.assertSeparate(
                tex._verts_uvs_list[i], tex_cloned._verts_uvs_list[i]
            )
            self.assertClose(
                tex._verts_uvs_list[i], tex_cloned._verts_uvs_list[i]
            )
    ...
    def test_detach(self):
        ...
        for i in range(tex._N):
            ...
            self.assertClose(
                tex._verts_uvs_list[i], tex_detached._verts_uvs_list[i]
            )
            self.assertFalse(tex_detached._faces_uvs_list[i].requires_grad)
            self.assertClose(
                tex._faces_uvs_list[i], tex_detached._faces_uvs_list[i]
            )
            # tex._maps_list is not use anywhere so it's not stored. We call it explicitly
    ...
    def test_centers_for_image(self):
        ...
        verts_uvs = torch.FloatTensor(
            [[[0.25, 0.125], [0.5, 0.625], [0.5, 0.5]]]
        )
...
class TestRectanglePacking(TestCaseMixin, unittest.TestCase):
    ...
    def wrap_pack(self, sizes):
        ...
        self.assertTrue(
            torch.all(torch.sum(mask, dim=0, dtype=torch.int32) > 0)
        )
        self.assertTrue(
            torch.all(torch.sum(mask, dim=1, dtype=torch.int32) > 0)
        )
        return res
