...
class SceneModel(nn.Module):
    ...
    def __init__(self):
        ...
        torch.manual_seed(1)
        vert_pos = (
            torch.rand(N_POINTS, 3, dtype=torch.float32, device=DEVICE) * 10.0
        )
        vert_pos[:, 2] += 25.0
        vert_pos[:, :2] -= 5.0
        self.register_parameter(
            'vert_pos', nn.Parameter(vert_pos, requires_grad=True)
        )
        self.register_parameter(
            'vert_col',
            nn.Parameter(
                torch.ones(N_POINTS, 3, dtype=torch.float32, device=DEVICE)
                * 0.5,
                requires_grad=True,
            ),
        )
        self.register_parameter(
            'vert_rad',
            nn.Parameter(
                torch.ones(N_POINTS, dtype=torch.float32) * 0.3,
                requires_grad=True,
            ),
        )
        self.register_buffer(
            'cam_params',
            torch.tensor(
                [0.0, 0.0, 0.0, 0.0, math.pi, 0.0, 5.0, 2.0],
                dtype=torch.float32,
            ),
        )
        ...
        raster_settings = PointsRasterizationSettings(
            image_size=(WIDTH, HEIGHT), radius=self.vert_rad
        )
...
def cli():
    ...
    for i in range(500):
        ...
        overlay_img = np.ascontiguousarray(
            ((result * 0.5 + ref * 0.5).cpu().detach().numpy() * 255).astype(
                np.uint8
            )[:, :, ::-1]
        )
